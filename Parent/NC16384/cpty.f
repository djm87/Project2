      ...
      COMMON /CPPROP/ PI,GDO,EXPM,SO,HO,SS,EXPA
      COMMON /CPDEF/DEF1(3,3),DEF2(3,3),DEFS1(3,3),DEFS2(3,3)
      COMMON /CPSTRESS/ STRESS2BE(6),STRV(6)
      COMMON /RSS2/ DRFCE2(12),DG2(12),DVELP(3,3)
	...
	t_1 = t
      ...
      FILENAME1 = '/home/administrator/Documents/DAN/itter_spectral_comp/Parent/timing.txt'
      FILENAME = 'stress.txt'
      parg = 'append'
      DO IC = 1,NC
      ...
      CALL POLYPLMDL(FILENAME,FILENAME1,FILENAME2,parg,IPOINT,NSTEP,IDEF,
     ...
     +                  DEFGRDZY,DEFGRDZZ,PHI1,PHI,PHI2,NC)
      ...
      SUBROUTINE POLYPLMDL(FILENAME,FILENAME1,FILENAME2,parg,IPOINT,NSTEP,IDEF,
     +                  DT,TEPS,DEFGRDXX,DEFGRDXY,DEFGRDXZ,
     +                  DEFGRDYX,DEFGRDYY,DEFGRDYZ,DEFGRDZX,
     +                  DEFGRDZY,DEFGRDZZ,PHI1,PHI,PHI2,NC)
      ...
      DIMENSION STRS2FEM(6),ESV(6),C6A(6,NSLIP),B6A(6,NSLIP),
     +          TEMP3(3,3,NSLIP),TEMP2(3,3),TEMP(3,3),FFFF(3,3)
      ...
      OPEN(14, file=FILENAME1,Access=parg)
...
!        PHI2(IC) = PHI2(IC)*PI/180.0D0 
	...
	t_1 = t
      ...
      CALL MATMATMLT(DEFPI1(1,1,IC), TEMP2(1,1), FFFF(1,1), 2) 


      ESV(1)=FFFF(1,1)-1.0D0
      ESV(2)=FFFF(2,2)-1.0D0
      ESV(3)=FFFF(3,3)-1.0D0
      ESV(4)=FFFF(1,2)*2.0D0
      ESV(5)=FFFF(2,3)*2.0D0
      ESV(6)=FFFF(1,3)*2.0D0 

      STRV(1) = 0.5D0*(ELSTIF(1,1,IC)*ESV(1) + ELSTIF(1,2,IC)*ESV(2) +
     +          ELSTIF(1,3,IC)*ESV(3) + ELSTIF(1,4,IC)*ESV(4) +
     +          ELSTIF(1,5,IC)*ESV(5) + ELSTIF(1,6,IC)*ESV(6))
      STRV(2) = 0.5D0*(ELSTIF(2,1,IC)*ESV(1) + ELSTIF(2,2,IC)*ESV(2) +
     +          ELSTIF(2,3,IC)*ESV(3) + ELSTIF(2,4,IC)*ESV(4) +
     +          ELSTIF(2,5,IC)*ESV(5) + ELSTIF(2,6,IC)*ESV(6))
      STRV(3) = 0.5D0*(ELSTIF(3,1,IC)*ESV(1) + ELSTIF(3,2,IC)*ESV(2) +
     +          ELSTIF(3,3,IC)*ESV(3) + ELSTIF(3,4,IC)*ESV(4) +
     +          ELSTIF(3,5,IC)*ESV(5) + ELSTIF(3,6,IC)*ESV(6))
      STRV(4) = 0.5D0*(ELSTIF(4,1,IC)*ESV(1) + ELSTIF(4,2,IC)*ESV(2) +
     +          ELSTIF(4,3,IC)*ESV(3) + ELSTIF(4,4,IC)*ESV(4) +
     +          ELSTIF(4,5,IC)*ESV(5) + ELSTIF(4,6,IC)*ESV(6))
      STRV(5) = 0.5D0*(ELSTIF(5,1,IC)*ESV(1) + ELSTIF(5,2,IC)*ESV(2) +
     +          ELSTIF(5,3,IC)*ESV(3) + ELSTIF(5,4,IC)*ESV(4) +
     +          ELSTIF(5,5,IC)*ESV(5) + ELSTIF(5,6,IC)*ESV(6))
      STRV(6) = 0.5D0*(ELSTIF(6,1,IC)*ESV(1) + ELSTIF(6,2,IC)*ESV(2) +
     +          ELSTIF(6,3,IC)*ESV(3) + ELSTIF(6,4,IC)*ESV(4) +
     +          ELSTIF(6,5,IC)*ESV(5) + ELSTIF(6,6,IC)*ESV(6))
      
      DO I = 1,NSLIP
C
      CALL MATMATMLT(FFFF(1,1), SSYSMAT(1,1,I,IC), TEMP3(1,1,I), 1)             
C
       B6A(1,I) = TEMP3(1,1,I)+TEMP3(1,1,I)
       B6A(2,I) = TEMP3(2,2,I)+TEMP3(2,2,I)
       B6A(3,I) = TEMP3(3,3,I)+TEMP3(3,3,I)
       B6A(4,I) = (TEMP3(1,2,I)+TEMP3(2,1,I))*2.0D0
       B6A(5,I) = (TEMP3(2,3,I)+TEMP3(3,2,I))*2.0D0       
       B6A(6,I) = (TEMP3(1,3,I)+TEMP3(3,1,I))*2.0D0             

       C6A(1,I)=0.5D0*(ELSTIF(1,1,IC)*B6A(1,I)+ELSTIF(1,2,IC)*B6A(2,I)+
     +            ELSTIF(1,3,IC)*B6A(3,I)+ELSTIF(1,4,IC)*B6A(4,I) +
     +            ELSTIF(1,5,IC)*B6A(5,I)+ELSTIF(1,6,IC)*B6A(6,I))
       C6A(2,I)=0.5D0*(ELSTIF(2,1,IC)*B6A(1,I)+ELSTIF(2,2,IC)*B6A(2,I)+
     +            ELSTIF(2,3,IC)*B6A(3,I)+ELSTIF(2,4,IC)*B6A(4,I) +
     +            ELSTIF(2,5,IC)*B6A(5,I)+ELSTIF(2,6,IC)*B6A(6,I))
       C6A(3,I)=0.5D0*(ELSTIF(3,1,IC)*B6A(1,I)+ELSTIF(3,2,IC)*B6A(2,I)+
     +            ELSTIF(3,3,IC)*B6A(3,I)+ELSTIF(3,4,IC)*B6A(4,I) +
     +            ELSTIF(3,5,IC)*B6A(5,I)+ELSTIF(3,6,IC)*B6A(6,I))
       C6A(4,I)=0.5D0*(ELSTIF(4,1,IC)*B6A(1,I)+ELSTIF(4,2,IC)*B6A(2,I)+
     +            ELSTIF(4,3,IC)*B6A(3,I)+ELSTIF(4,4,IC)*B6A(4,I) +
     +            ELSTIF(4,5,IC)*B6A(5,I)+ELSTIF(4,6,IC)*B6A(6,I))
       C6A(5,I)=0.5D0*(ELSTIF(5,1,IC)*B6A(1,I)+ELSTIF(5,2,IC)*B6A(2,I)+
     +            ELSTIF(5,3,IC)*B6A(3,I)+ELSTIF(5,4,IC)*B6A(4,I) +
     +            ELSTIF(5,5,IC)*B6A(5,I)+ELSTIF(5,6,IC)*B6A(6,I))
       C6A(6,I)=0.5D0*(ELSTIF(6,1,IC)*B6A(1,I)+ELSTIF(6,2,IC)*B6A(2,I)+
     +            ELSTIF(6,3,IC)*B6A(3,I)+ELSTIF(6,4,IC)*B6A(4,I) +
     +            ELSTIF(6,5,IC)*B6A(5,I)+ELSTIF(6,6,IC)*B6A(6,I))
     
      ENDDO
      
      STRESS2BE(1) = STRS1PK(1,IC)
      STRESS2BE(2) = STRS1PK(2,IC)
      STRESS2BE(3) = STRS1PK(3,IC)
      STRESS2BE(4) = STRS1PK(4,IC)
      STRESS2BE(5) = STRS1PK(5,IC)
      STRESS2BE(6) = STRS1PK(6,IC)
      
      CALL CPSOLV(IC,IIPT,IC,ISOLER,DT,C6A,SLPRA1,SLPRA2,
     +                 SSYSMAT, 1, STRS1PK)
...
C---+----+----+----+----+----+----+----+----+----+----+----+----+----+--
      SUBROUTINE CPSOLV(IC,IIPT,IELM,ISOLER,DTIME,C6A,SLPRA1,SLPRA2,
     +                  SSYSMAT,KUPD,STRS1PK)
      ...
      COMMON /RSS2/ DRFCE2(12),DG2(12),DVELP(3,3)
      COMMON /ELBRICK/ NIPT
      COMMON /C6P6AMAT/ C6P6A(6,6,NSLIP)
     ...
     +          C6A(6,NSLIP),SSYSMAT(3,3,NSLIP,*),STRS1PK(6,*)
...
C 
      ...
      ENDDO       
C 
C     INITIALISE SLIP RESISTANCE AND GET CONSTANT C6P6A
      DO I = 1,NSLIP
         SLPRESA2(I)= SLPRA1(I,IC) 
          DO II = 1,6 
            C6P6A(II,1,I) = C6A(II,I)*SSYSMAT(1,1,I,IC)            
            C6P6A(II,2,I) = C6A(II,I)*SSYSMAT(2,2,I,IC)
            C6P6A(II,3,I) = C6A(II,I)*SSYSMAT(3,3,I,IC)
            C6P6A(II,4,I) = C6A(II,I)*(SSYSMAT(1,2,I,IC)+
     +                                   SSYSMAT(2,1,I,IC))
            C6P6A(II,5,I) = C6A(II,I)*(SSYSMAT(2,3,I,IC)+
     +                                   SSYSMAT(3,2,I,IC))
            C6P6A(II,6,I) = C6A(II,I)*(SSYSMAT(1,3,I,IC)+
     +                                   SSYSMAT(3,1,I,IC))
...
C
      DO JJ = 1,NSLIP
           DRFCE2(JJ) = DRFCE2(JJ)+STRESS2BE(1)*SSYSMAT(1,1,JJ,IC)
           DRFCE2(JJ) = DRFCE2(JJ)+STRESS2BE(2)*SSYSMAT(2,2,JJ,IC)
           DRFCE2(JJ) = DRFCE2(JJ)+STRESS2BE(3)*SSYSMAT(3,3,JJ,IC)
           DRFCE2(JJ) = DRFCE2(JJ)+STRESS2BE(4)*(SSYSMAT(1,2,JJ,IC)+
     +                                        SSYSMAT(2,1,JJ,IC))
           DRFCE2(JJ) = DRFCE2(JJ)+STRESS2BE(5)*(SSYSMAT(2,3,JJ,IC)+
     +                                        SSYSMAT(3,2,JJ,IC))
           DRFCE2(JJ) = DRFCE2(JJ)+STRESS2BE(6)*(SSYSMAT(1,3,JJ,IC)+
     +                                        SSYSMAT(3,1,JJ,IC))
...
C
      ...
      DO J=1,6 
	  TPKOLD(J) = STRESS2BE(J)
      ENDDO

      DO J=1,6
         EROR(J) =  STRESS2BE(J) - STRV(J)
      ENDDO
C
      DO I=1,NSLIP
           DO II=1,6
             EROR(II) = EROR(II)+DG2(I)*C6A(II,I)
           ENDDO
      ENDDO
C          
      DO K=1,6
      ...
      DO I = 1,NSLIP
           IF (DG2(I).NE.0.0D0) THEN
             DGDTPKA(I) = DG2(I)*EXPN/DRFCE2(I)
               DO K = 1,6
                 DO KK = 1,6
...
C     GET STRESS CORRECTION AND CORRECT PK STRESS
...
C
      DO JJ = 1,6
         ...
         STRESS2BE(JJ) = STRESS2BE(JJ) - DELTPK(JJ)
      ENDDO
C
      ERORDELTPK = 0.0D0
      DO J = 1,6
...
C      
C     END OF SLIP RESISTANCE LOOP
C
        STRS1PK(1,IC)=STRESS2BE(1)
	STRS1PK(2,IC)=STRESS2BE(2)
	STRS1PK(3,IC)=STRESS2BE(3)
	STRS1PK(4,IC)=STRESS2BE(4)
	STRS1PK(5,IC)=STRESS2BE(5)
	STRS1PK(6,IC)=STRESS2BE(6)

      DGMAX = 0.0D0
C
      IF(KUPD.EQ.1)THEN
        DO I = 1,NSLIP
          SLPRA2(I,IELM)=SLPRESA2(I)
          SLPRA1(I,IELM)=SLPRESA2(I)
...
C     CONTAINS THE CONSTANT VECTOR WHEN MATINV IS CALLED, AND THIS IS  *
...
C---+----+----+----+----+----+----+----+----+----+----+----+----+----+--
     ...
     +                 STRS2FEM)
      ...
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER (I-N)
      PARAMETER(NSLIP=12)
      COMMON /CPDEF/DEF1(3,3),DEF2(3,3),DEFS1(3,3),DEFS2(3,3),DETDEFS2
      COMMON /CPSTRESS/ STRESS2BE(6),STRV(6)
      COMMON /RSS2/ DRFCE2(12),DG2(12),DVELP(3,3)
     ...
     +          TEMP(3,3),STRS2FEM(6)
      ...
      DVELP(3,3) = 0.0D0
      DO I = 1,NSLIP
        DVELP(1,1) = DVELP(1,1)+DG2(I)*SSYSMAT(1,1,I,IC)
        DVELP(1,2) = DVELP(1,2)+DG2(I)*SSYSMAT(1,2,I,IC)
        DVELP(1,3) = DVELP(1,3)+DG2(I)*SSYSMAT(1,3,I,IC)

        DVELP(2,1) = DVELP(2,1)+DG2(I)*SSYSMAT(2,1,I,IC)
        DVELP(2,2) = DVELP(2,2)+DG2(I)*SSYSMAT(2,2,I,IC)
        DVELP(2,3) = DVELP(2,3)+DG2(I)*SSYSMAT(2,3,I,IC)
        DVELP(3,1) = DVELP(3,1)+DG2(I)*SSYSMAT(3,1,I,IC)
        DVELP(3,2) = DVELP(3,2)+DG2(I)*SSYSMAT(3,2,I,IC)
        DVELP(3,3) = DVELP(3,3)+DG2(I)*SSYSMAT(3,3,I,IC)
      ENDDO        
...
C
      ...
      STRESPK(1,1)=STRESS2BE(1) 
      STRESPK(1,2)=STRESS2BE(4) 
      STRESPK(2,3)=STRESS2BE(5)
      STRESPK(2,1)=STRESS2BE(4) 
      STRESPK(2,2)=STRESS2BE(2) 
      STRESPK(1,3)=STRESS2BE(6)
      STRESPK(3,2)=STRESS2BE(5) 
      STRESPK(3,1)=STRESS2BE(6) 
      STRESPK(3,3)=STRESS2BE(3)
...
C---+----+----+----+----+----+----+----+----+----+----+----+----+----+--      
     ...
     +          PHI1OUT(NUMEL),PHIOUT(NUMEL),PHI2OUT(NUMEL)   
      ...
      PI = 4.0D0*DATAN(1.0D0)      
c      OPEN (29,FILE = 'ODFOUT.TXT',ACCESS= 'APPEND',STATUS = 'UNKNOWN')           
      DO J = 1,NUMEL
...
C           MAKE SURE ANGLES ARE POSITIVE
...
c         WRITE(29,559)180.0D0/PI*ANGLES(1),180.0D0/PI*ANGLES(2),
c     +          180.0D0/PI*ANGLES(3)
	PHI1OUT(J) = ANGLES(1)
	PHIOUT(J) = ANGLES(2)
        PHI2OUT(J) = ANGLES(3)
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER (I-N)
      COMMON/VECT5/
     1 F11,F22,F33,F12,F13,F21,
     2 F23,F31,F32,C11,C22,C33,
     3 C12,C13,C23,C1212,C2323,C1313,
     4 ALI1,ALI2,ALI3,UI1,UI2,UI3,
     5 S11,S22,S33,S12,S13,S23,
     6 UI11,UI22,UI33,UI12,UI13,UI23,
     7 SI11,SI22,SI33,SI12,SI13,SI23,
     8 AMDA1,AMDA2,AMDA3,R,Q,SDETM,
     9 XMOD,UI1S,SCL1,SCL2,SCL0,CT3,ST3,
     & C2313,C1223,C1213
...
C
      ...
      S11=C22*C33-C2323
      UI1=O3*(C11+C22+C33)  
C      UI2=S11+C11*C22+C33*C11-C1212-C1313
C      UI3=C11*S11+C12*(C2313-C12*C33)
C     1      +C13*(C1223-C22*C13)
...
C      AMDA1=SQRT(2.000*(CT3+SDETM)+UI1)
C      AMDA2=SQRT(-CT3+ST3-SDETM+UI1)
C   50 AMDA3=SQRT(-CT3-ST3-SDETM+UI1)
C
      SDETM=AMDA1*AMDA2
