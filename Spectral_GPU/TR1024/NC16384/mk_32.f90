module SCP_VARIABLES
  ...
  integer, parameter :: SUP_IND = 1024, NCRYS = 10, m_fine = 3000 !change, added m_fine
...
!  CLOSE(33)
   ...
   Super_set_1=Super_set_out(:,1)  
   Super_set_2=Super_set_out(:,2)  
   Super_set_3=Super_set_out(:,3)  
   Super_set_4=Super_set_out(:,4)  
...
!
  ...
  fact_2 = fact_1 * 1.d-02 / dfloat(NCRYS) * sign((eps_dot/DGo)**mo, eps_dot/DGo)
...
t_1 = t
  ...
  do j = 1,n_steps
     ...
     DO i = 1,NCRYS
        ...
        !change, start
        SC = 2*pi*mod(((Super_set_1-1) * Phi2(i) &
             + (Super_set_2-1) * PHI(i) &
             + (Super_set_3-1) * Phi1(i) &
             + (Super_set_4-1) * termsdim4(i)),sizeF(1))/sizeF(1)
        !change, end
